...
def plot_sector_grid(sector_name, sector_df, year_col):
    # --- LOGIC FOR POWER & STORAGE FILTRATION ---
    # --- CHANGE THIS SECTION ---
    if sector_name in ["Power", "Storage"]:
    # Add .copy() at the end here
        sector_df = sector_df[sector_df['output_comm'].isin(['ELC_CEN', 'ELC_DST'])].copy()
    ...
    # Identify Reference rows (Anchors)
    ...
    # Calculate Global Y-Limit (Pivoting to handle the summation of ELC_CEN + ELC_DST)
    ...
    for i, scen in enumerate(comparison_scenarios):
        ...
        # Combine Reference + Current Scenario
        combined = pd.concat([ref_csd, scen_subset, ref_nze])
        
        # Summing values for the same group across different tech/output_comm
        ...
        probs = sorted([p for p in pivot_df.index if p not in ["NZE", "CSD"]])
        ...
        if i % cols == 0:
            ax.set_ylabel(f"Output [{unit}]", fontsize=8)
        else:
            ax.set_ylabel("")
    ...
    # LEGEND: Bottom-right corner in 2 columns
    if legend_handles:
    ...
    fig.suptitle(f"{sector_name} Sector Output Mix [{unit}] - {year_col}", 
                 fontsize=14, fontweight='bold', x=0.5, y=0.96)
    
    plt.savefig(f"{sector_name}_FlowOut_Grid_{year_col}.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    for sector in SECTORS_TO_PLOT:
        try:
            ...
            # Ensure all columns are strings (years can sometimes load as ints)
            ...
            for year in YEAR_COLS:
                if year in df_sector.columns:
                    ...
                    print(f"Generated Flow Out chart: {sector} {year}")
        except Exception as e:
            print(f"Error in {sector}: {e}")

    print("\n✅ Flow Out processing complete.")