...
def get_combined_heat_data():
    """Reads all sheets and filters for HET to create a combined sector dataset."""
    all_data = []
    
    # Iterate through the Excel sheets based on the provided mapping
    ...
    for sheet_name, display_name in SHEET_TO_SECTOR.items():
        if sheet_name in xls.sheet_names:
            ...
            # Filter for Heat Input (HET)
            df_het = df[df['input_comm'] == 'HET'].copy()
            if not df_het.empty:
                # Assign the display name as the 'Sector' for plotting
                df_het['Sector_Name'] = display_name
                all_data.append(df_het)
...
def plot_heat_usage_grid(combined_df, year_col):
    # References for Anchors
    ...
    if not comparison_scenarios:
        print(f"No comparison scenarios found for {year_col}")
        return

    # Unified Scaling: Summing values by scenario and Probability to find max stack height
    ...
    for i, scen in enumerate(comparison_scenarios):
        ...
        # Pivot by Sector_Name for the stacked bar
        ...
        ax.set_xlabel("")
        if i % cols == 0:
            ax.set_ylabel("Heat Use [PJ]", fontsize=8)

        if i == 0:
            legend_handles, legend_labels = ax.get_legend_handles_labels()

    # Two-column Legend in bottom-right
    if legend_handles:
    ...
    fig.suptitle(f"Heat Use Mix across Sectors [PJ] - {year_col}", 
                 fontsize=14, fontweight='bold', x=0.5, y=0.96)
    
    plt.savefig(f"Heat_Sector_Use_Mix_{year_col}.png", dpi=300)
    plt.close()

# --- EXECUTION ---
if __name__ == "__main__":
    print("Combining heat consumption data from all sheets...")
    combined_heat_df = get_combined_heat_data()
    
    if not combined_heat_df.empty:
        for year in YEAR_COLS:
            if year in combined_heat_df.columns:
                plot_heat_usage_grid(combined_heat_df, year)
                print(f"Generated Sector Mix chart for {year}")
    else:
        print("Error: No HET data found in the specified sheets.")

    print("\n✅ Heat Sector Mix processing complete.")